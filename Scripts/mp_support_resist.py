...
def filter_consecutive_value(data: pd.DataFrame, signal_column: str):
    filtered_signal = data[signal_column].copy()
    for i in range(1, len(filtered_signal)):
        if filtered_signal[i] == filtered_signal[i - 1]:  # Si le signal est le même que le précédent
            filtered_signal[i] = 0  # Supprimer le signal en le mettant à 0 (ou NaN)
